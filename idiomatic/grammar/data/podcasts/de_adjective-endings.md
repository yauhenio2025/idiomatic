...
episode: 6
lang: de
title: "Adjective endings: one decision tree"
est_minutes: 15
evidence_refs:
...
Today we are going to turn German adjective endings into one spoken decision tree. Not three tables to admire, and not sixteen little guesses to make from sound. One decision tree that produces the article, the adjective ending, and the noun phrase as a unit.
...
TL: der härteste Konkurrent

TL: sehr tiefgründige Schlüsse

TL: kein kohärentes Weltbild

TL: mein ultimatives Ziel

The pattern survived for five years because an adjective ending is the last visible result of several earlier decisions. If one of those decisions is fuzzy, the final syllable feels arbitrary. So we will stop starting with the adjective. We will start where the grammar engine starts: with the noun phrase's features.

Here is the entire route in advance. First, identify gender and number. Second, identify case. Third, inspect the determiner family. That chooses weak, mixed, or strong adjective declension. Fourth, attach the ending. Finally, check whether the noun itself changes. Gender or number, then case, then determiner family, then adjective, then noun. We will walk that route slowly and then speed it up.

Decision one is gender and number. In the singular, retrieve the noun with its gender. Do not let Russian gender vote. The record shows exactly that pressure with the feminine Russian word for goal pulling German in the wrong direction. German says:
...
TL: Es kommen noch zwölf weitere.

Decision two is case. Use meaning and sentence structure before morphology. Which noun phrase is the grammatical subject, the phrase the finite verb agrees with? That is normally nominative; in an active clause it is often the actor. What is directly affected by an ordinary transitive verb? Usually accusative. Who receives or benefits, or which phrase is selected by a dative verb or dative preposition? Dative. Whose or of what, especially in a written noun chain? Genitive.

Take one noun through three roles. Listen to how the role changes the whole phrase.

TL: Der neue Bericht erscheint heute.

The report is the subject, so nominative.

TL: Die Redaktion prüft den neuen Bericht.

The report is the direct object, so accusative.

TL: Die Redaktion arbeitet mit dem neuen Bericht.

The preposition requires dative. The vocabulary barely changes; the grammatical job does.

Here is self-check one. Say, “The new program works.” Use a definite article. Diagnose before speaking: neuter singular, subject, definite family.

[PAUSE]

TL: Das neue Programm funktioniert.
...
TL: kein neues Programm

TL: meine neue Agentur

If there is no article or determiner, choose the strong pattern. Now the adjective must carry nearly all the visible case, gender, and number information itself.

TL: neuer Bericht

TL: mit neuem Material
...
TL: das neue Programm

TL: die neue Studie lesen

TL: das neue Programm testen

Now hear the longer ending in masculine accusative, every dative and genitive cell, and all plurals.

TL: den neuen Bericht

TL: mit der neuen Studie

TL: wegen des neuen Programms

TL: die neuen Berichte

This corrects the first fossil. The article already says masculine nominative, so the adjective does not repeat the article's ending.

TL: der härteste Konkurrent

Here is self-check two. Say, “We discussed the important question.” The question is feminine, the phrase is accusative, and the definite article is present.
...
TL: Wir brauchen ein neues Ziel.

That last cell explains two of the recorded problems. The nouns are neuter, and the possessive or negative determiner leaves the neuter signal unstated. The adjective has to provide it.
...
TL: einen neuen Bericht

TL: mit einem neuen Bericht

TL: meine neuen Bücher

There is a useful precision here. The memory image is one clear signal, not literally one and only one ending in the phrase. German can repeat some information. The point is to ask whether the determiner leaves the decisive gender-and-case cell exposed. If it does, the mixed adjective fills the gap. If it does not, the adjective takes the quiet form.

Self-check three. Repair the attested idea and say, “My ultimate goal is clear.”

[PAUSE]

TL: Mein ultimatives Ziel ist klar.

The strong branch is what happens when there is no determiner. Here the adjective resembles the ending of the corresponding definite article. That gives a compact audible map. In nominative singular, hear masculine, feminine, and neuter like this:
...
TL: zwölf weitere Kapitel

That repairs two more fossils. In dative without an article, the adjective has to show the dative distinction.

TL: mit gutem Grund

TL: mit klarer Sprache

TL: mit kaltem Wasser

TL: mit neuen Ideen

The genitive has a wrinkle: masculine and neuter adjectives normally take the longer ending because the noun itself carries the genitive signal.

TL: wegen guten Wetters

That is one reason not to force the slogan “copy the article” beyond its useful range. It is a navigation aid, not the grammar itself.

Here is self-check four. Say, “Critical analyses can change the debate.” There is no determiner; the plural noun phrase is the subject.
...
Suppose you want “a convincing argument” as the subject. Argument is neuter singular. Subject means nominative. The indefinite article has no neuter ending, so the mixed adjective fills the gap.
...
Self-check five. Say, “They are working with new methods.” Use no determiner.

[PAUSE]

TL: Sie arbeiten mit neuen Methoden.

The practical training goal is not to recite a matrix. It is to make the first three decisions fast enough that the ending no longer arrives as a surprise. On a walk, take one useful noun phrase and rotate only one feature at a time.
...
Then change the determiner branch while keeping the noun and role stable.

TL: ein neuer Bericht

TL: unser neuer Bericht
...
TL: Der Bericht ist neu.

An adverbial adjective is uninflected as well.
...
TL: ein neuer, wichtiger Bericht

TL: mit neuen, verlässlichen Daten
