...
async def generate_batch(topic: Topic, n: int,
    ...
    for item in raw:
        ...
        if ok:
            seen_keys.add(key)
            # Passive is deliberately hybrid per item. Known participles have
            # already completed Tier A and go straight to accepted; only an
            # unknown lexical participle falls back to K=3 blind solvers.
            item_needs_blind = (
                needs_blind
