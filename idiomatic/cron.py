...
async def run() -> None:
    ...
    # One client for the entire walk — avoids new TCP+TLS handshake per
    # request and lets httpx reuse the connection pool.
    async with httpx.AsyncClient(timeout=20, follow_redirects=True,
        ...
        for i, ch in enumerate(channels):
            ...
            for e in entries:
                try:
                    meta = await fetch_metadata(e.youtube_id, client=client)
                except Exception as ex:
                    log.warning("cron.meta_failed", yt=e.youtube_id,
                                 err=str(ex)[:200])
                    continue
                ...
                if vid is None:
                    ...
                    log.info("cron.enqueued", id=vid, yt=e.youtube_id,
                              lang=ch["lang"], dur=dur,
                              title=meta["title"][:60])

            # Free heavy per-channel buffers (httpx response bodies, regex
            # match objects) before moving on to the next channel.
            gc.collect()
